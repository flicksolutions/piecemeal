...
\begin{enumerate}
    ...
    %T_i^*:= \forall s \in S : \{T_i \cup s_1, ..., T_i \cup s_{\lvert S \rvert} \}
    $$
    ...
    %T_i^* \bigcup \forall e \in T_i : \{(T_i \setminus \{e_1\})\cup \{\neg e_1\}, ..., T_i \setminus \{e_{\lvert T_i \rvert}\})\cup \{\neg e_{\lvert T_i \rvert}\}\}
    $$
    ...
    %T_i^* \bigcup \forall e \in T_i : \{T_i \setminus \{e_1\}, ..., T_i \setminus \{e_{\lvert T_i \rvert}\}\}
    $$
...
\begin{enumerate}
    ...
    $$
    \item Jede Menge von Sätzen, die entsteht, wenn man einen einzelnen Satz in $C_i$ negiert, wird zu $C_i^*$ hinzugefügt.
    $$
...
\begin{enumerate}
    ...
    \item \label{i4} Falls $z < \lvert C_i \rvert$: Das erste Element in $C_i$ wird nach hinten verschoben. Dabei wird die erste Überzeugung zur letzten. $z$ wird um eins inkrementiert: $z = z + 1$ und zu Schritt \ref{i1} gesprungen. %Achtung: Vielleicht müssen alle Permutationen von C_i geprüft werden? Es macht definitiv einen unterschied... die Frage ist, ob es auch schlechter funktioniert...
