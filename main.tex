...
\paragraph{Spezialfall} Bei der ersten Theorieanpassung wird immer die leere Menge ausgewählt.\footnote{Dies ist notwendig, da das programmierte Modell so funktioniert}
...
$$
 ...
 Von all den Überzeugungsmengen in $C_1^{***}$ haben folgende 3 Mengen den gleichen Wert der Achievement-Funktion: $\{s3,s4,s6,s1\}$, $\{s3,s4,s6,\neg s2\}$, $\{s3,s4,s6,\neg s6\}$. Ob man also $s1$ zu den weiteren Überzeugungen dazunimmt oder $\neg s2$ oder $\neg s6$ hinzunimmt spielt für die nächste Generation keine Rolle. In allen drei Optionen wird nämlich das Desideratum der Kohärenz etwas besser erfüllt und das Desideratum des Respekts etwas weniger gut, denn wir haben einen Satz in der Überzeugungsmenge, der nicht zur ursprünglichen Überzeugungsmenge gehört. 
Zur Einfachheit des Beispiels gehe ich davon aus, dass zufällig eine die Möglichkeit gewählt wird, $\neg s2$ zu den Überzeugungen hinzuzufügen. Ich komme allerdings im Abschnitt \ref{branching} auf die Möglichkeit des \textit{Branching} zu sprechen. $C_1 = \{s3, s4, s5, \neg s2\}$
