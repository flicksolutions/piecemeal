...
Ich habe bisher den Begriff der Konsistenz schon einige mal verwendet, ohne genau zu definieren, was dieser bedeutet. Das möchte ich nun ändern. Es gibt zwei Arten von Konsistenz:

\textit{\textcolor{red}{Hier folgt noch eine Einführung der fehlenden Begriffe für den folgenden Teil,  wie beispielsweise der Achievement-Funktion. Auch Branching und Konsistenz werden noch als Begriffe eingeführt}}
...
Zusätzlich schränken nun die Inferenzbeziehungen zwischen den Sätzen, die Kandidaten ein. Weil Theorien vollständig konsistent sein müssen, darf auch kein Theoriekandidat Inkonsistenzen enthalten. Wenn also eine bspw. der Satz $s3$ aus $s1$ folgt und aus $s2$ folgt $\neg s2$, so kann man auf den Theoriekandidaten ${s1,s2}$ verzichten, denn der Inhalt dieser Theorie wäre ${s1,s2,s3,\neg s3}$. Er enthält also zwei Sätze, die sich widersprechen ($s3$ und $\neg s3$). Somit ist dies gar kein Theoriekandidat.
