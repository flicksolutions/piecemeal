...
\paragraph{Anpassung der Theorie} \label{T-Anpassung}
...
\begin{enumerate}
    ...
    \item \label{4} Der Theoriekandidat, welcher den höchsten Wert bei $Z$ erreicht, wird zur Theorie $T_{i+1}$ erklärt, falls sein $Z$-Wert auch grösser ist als jener von $T_i$ - Ansonsten wird $T_i$ zu $T_{i+1}$.
\end{enumerate}

\paragraph{Anpassung der Überzeugungen} \label{C-Anpassung}
...
\end{figure}
 ...
 Als Beispiel für den Ablauf kann die klassische Struktur von \cite{beisbart_making_2015} im initialen Zustand mit $C_0 = \{s3, s4, s5\}$ zur Hand genommen werden. Sie ist auf Abbildung \ref{fig:classset-initial1} dargestellt. Zur ersten Anpassung der Theorie - Also um $T_1$ zu finden - wird nun die initiale Theorie genommen, welche die leere Menge ist\footnote{Einen Vorschlag für eine bessere initiale Theorie befindet sich \hyperref[better-first-theory]{im Anhang}.}, und ihr werden einzeln alle Sätze des Themas S hinzugefügt.
 $$
 T_0: \{\} \longrightarrow T_0^*: \{\{s1\},...,\{s7\}, \{\neg s1\}, ..., \{\neg s7\}\}
 $$
 Beim \hyperref[1.1]{zweiten Schritt} kommt keine Menge hinzu, denn in der leeren Menge gibt es keine Sätze, die negiert werden könnten. Auch beim \hyperref[2]{dritten Schritt} kommen keine neuen Sätze hinzu, denn man kann keine Sätze aus der leeren Menge entfernen. Auch im \hyperref[konsistenzfilter]{vierten Schritt} können keine Mengen entfernt werden, denn eine Menge mit einem Element kann nie inkonsistent sein. Die erste Anpassung der Theorie überprüft also einfach alle einzelnen Sätze $\{s1,...,s7, \neg s1, ..., \neg s7\}$. Derjenige, welche alleine den höchsten Wert von $Z$ erreicht, wird zum einzigen Prinzip. Dies ist $s1$, weil er alle Überzeugungen rechtfertigt und damit wird $\{s1\}$ zu $T_1$.
 ...
 \begin{equation} \notag
 \begin{split}
 C_1^*: \{\{s3, s4, s5, \textcolor{red}{s1}\}, \{s3, s4, s5, \textcolor{red}{s2}\}, \{s3, s4, s5, \textcolor{red}{s6}\}, \{s3, s4, s5, \textcolor{red}{s7}\}, \\
 \{s3, s4, s5, \textcolor{red}{\neg s1}\}, \{s3, s4, s5, \textcolor{red}{\neg s2}\}, \{s3, s4, s5, \textcolor{red}{\neg s6}\}, \{s3, s4, s5, \textcolor{red}{\neg s7}\}\}
 \end{split}
 \end{equation}
